...
2. 先验证：
   ...
   ```powershell
   node "<技能根目录>/move-cocos-asset/scripts/move-asset.mjs" --project "<工程目录>" --from <原资源路径> --to <目标资源路径> --dry-run
   ```

3. 执行移动：

   ```powershell
   node "<技能根目录>/move-cocos-asset/scripts/move-asset.mjs" --project "<工程目录>" --from <原资源路径> --to <目标资源路径>
   ```
...
若元数据移动失败，脚本会尝试将资源移回原位置。回滚也失败时会报告两次错误及资源、元数据的位置；先核对并恢复成对状态，不直接重试或启动 Creator 导入。
